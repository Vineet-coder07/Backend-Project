...
const refreshAccessToken=asyncHandler(async (req,res)=>{
  ...
  try {
   const decodedToken= jwt.verify(
    ...
    throw new ApiError(500,"Something went wrong while accessing the access token")
  }
})
...
const updateAccountDetails =asyncHandler(async(req,res)=>{
  ...
  if(!avatar.url){
    throw new ApiError(500,"Something went while uploading avatar")
  }
  await User.findByIdAndUpdate(
    req.user?._id,
    {
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
  ...
  res.status(200).json(new ApiResponse(200, user, "Avatar file updated successfully"));
});

const getUserChannelProfile = asyncHandler(async (req, res) => {
  // TODO: implement fetching a user's channel profile
  ...
  const channel=await User.aggregate(
    ...
    },
    {
      $addFields:{
        ...
        isSubscribed:{
          $cond:{
            if:{$in:[req.user?._id,"$susbscribers.subsciber"]},
            then:true,
            else:false,
          }
        }
